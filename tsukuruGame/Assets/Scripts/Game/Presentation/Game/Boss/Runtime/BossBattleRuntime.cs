...
namespace Game.Presentation.Game.Boss.Runtime
{
    internal sealed class BossBattleRuntime : IDisposable
    {
        ...
        private void ResolveBossActionHitboxDamage()
        {
            IReadOnlyList<BossActiveHitbox> activeHitboxes = _context.BossActionFrameState.ActiveHitboxes;
            if (activeHitboxes == null || activeHitboxes.Count == 0)
                return;

            for (int i = 0; i < activeHitboxes.Count; i++)
            {
                BossActiveHitbox hitbox = activeHitboxes[i];
                NumericsVector3 hitboxPosition = _context.Boss.Position + hitbox.Offset;
                if (!AreOverlapping(
                        _context.Player.Position,
                        BossBattleRuntimeConstants.PlayerHitboxRadius,
                        hitboxPosition,
                ...
                if (TryApplyDamage(hitbox.Damage))
                    return;
            }
        }
